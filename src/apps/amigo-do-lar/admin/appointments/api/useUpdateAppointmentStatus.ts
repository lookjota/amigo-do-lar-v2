...
export function useUpdateAppointmentStatus(id: string) {
  ...
  return useMutation({ mutationFn: (input: UpdateAppointmentStatusInput) => updateAdminAppointmentStatus(id, input), retry: false, onSuccess: async (updated) => { client.setQueryData([...adminAppointmentsKey, 'detail', id], updated); await invalidateAppointmentQueries(client, updated.serviceRequestId) } })
}
